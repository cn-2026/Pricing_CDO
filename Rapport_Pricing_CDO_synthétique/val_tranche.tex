...
\subsection{Expected Loss et distribution des pertes}
...
\begin{equation}
\text{EL}_{\text{tranche}} = \mathbb{E}^*[L_{\text{tranche}}(T)] = \mathbb{E}^*\left[\min\left(\max(L(T) - K_1 N_{\text{tot}}, 0), (K_2 - K_1) N_{\text{tot}}\right)\right].
\end{equation}
...
\begin{equation}
\text{JF}(s) = s \sum_{i=1}^m \delta_i    \mathbb{E}^{\mathbb{P}^\ast}[ N_{t_i} N_{\text{restant}}(t_i)],
\end{equation}
où $\delta_i = t_i - t_{i-1}$ est la fraction d'année entre deux paiements (typiquement $\Delta t_i = 0.25$
pour une fréquence trimestrielle) et $N_{t_i} = e^{-\int_0^{t_i} r(s)ds}$ est le numéraire. En substituant
l'expression du notionnel restant, nous obtenons
\begin{equation}
\text{JF}(s) = s \sum_{i=1}^m \delta_i   \mathbb{E}^{\mathbb{P}^\ast}[N_{t_i}] \left[(K_2 - K_1) N_{\text{tot}} - \mathbb{E}^{\mathbb{P}^*}[L_{\text{tranche}}(t_i)]\right].
\end{equation}
...
\begin{equation}
\text{JV} = \mathbb{E}^{\mathbb{P}^\ast}\left[\int_0^T N_t \, dL_{\text{tranche}}(t)\right],
\end{equation}
...
\begin{equation}
\text{JV} \approx \sum_{i=1}^m  \mathbb{E}^{\mathbb{P}^\ast}[ N_{t_i}(L_{\text{tranche}}(t_i) - L_{\text{tranche}}(t_{i-1}))].
\end{equation}
Cette expression montre que la valeur de la jambe protection dépend des variations incrémentales 
de la perte espérée de la tranche entre les dates successives.
...
\begin{equation}
s^* \approx \frac{\sum_{i=1}^m  \mathbb{E}^{\mathbb{P}^\ast}[N_{t_i} (L_{\text{tranche}}(t_i) - L_{\text{tranche}}(t_{i-1}))]}{\sum_{i=1}^m \delta_i  \mathbb{E}^{\mathbb{P}^\ast}[N_{t_i}] [(K_2 - K_1) N_{\text{tot}} - \mathbb{E}^{\mathbb{P}^\ast}[L_{\text{tranche}}(t_i)]]}.
\end{equation}